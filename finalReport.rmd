...
<em style='color: black;'>Figure 10: Distribution of key engagement metrics across users, showing individual violin plots for each metric with median and interquartile range (IQR) statistics. Each plot uses a distinct color and includes summary statistics.</em>
...
<em style='color: black;'>Figure 11: Stacked bar graph showing the frequency of user-selected sentences in each sentiment category, stacked by language mode (top 5 languages shown in color, all others in grey).</em>
...
<em style='color: black;'>Figure 12: Top terms for each topic identified by LDA topic modeling of user-selected sentences. Each panel shows the most important words for one topic, with x-axis numbering visible for all.</em>
...
<em style='color: black;'>Figure 13: Correlation matrix of user features and engagement metrics. Circle size and color indicate the strength and direction of the correlation (purple = positive, orange = negative).</em>
...
<em style='color: black;'>Figure 16: K-means clustering of users based on engagement metrics. Each point represents a user, colored by cluster (Active or Occasional) in PCA space.</em>
...
kable(kmeans_summary, caption = "Table 20: Summary statistics for k-means-defined active and occasional users")
```


K-means clustering provides a data-driven segmentation of users based on their engagement patterns. The 'Active' cluster contains users with higher averages across all key metrics, while the 'Occasional' cluster includes users with lower engagement. This approach avoids arbitrary cutoffs and reflects natural groupings in the data. The results confirm that a small subset of users are highly engaged, while most interact with the platform at a lower level. This segmentation can be used for targeted feature development and user retention strategies.
...
table(enhanced_profiles$user_group3)
...
kable(user_group3_summary, caption = "Table 23: Summary statistics for Very Active, Active, and Regular users")
```
...
kable(ord_tidy, digits = 3, caption = "Table 24: Ordinal regression coefficients for user group score (2 = Very Active, 1 = Active, 0 = Regular), with p-values")

# Variable importance plot
ord_tidy$importance <- abs(ord_tidy$estimate)
ggplot(ord_tidy[ord_tidy$term != "(Intercept)", ], aes(x = reorder(term, importance), y = importance)) +
  geom_col(fill = jargon_purple) +
  coord_flip() +
  labs(title = "Variable Importance for User Group Score", x = "Feature", y = "Importance (|Coefficient|)") +
  theme_minimal()
```

We use ANOVA and chi-squared tests to compare features across user groups, and an ordinal regression to identify which features are most associated with higher engagement (from Regular [0] to Very Active [2]). Positive coefficients in the ordinal regression indicate features that increase the likelihood of being in a higher activity group. The variable importance plot shows the relative influence of each feature on user group score.