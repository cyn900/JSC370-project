...
The analysis uses real-life usage data collected from Jargon's Supabase database on March 16, 2025. The data represents all user interactions from the launch in June 2024 through March 16, 2025.
...
The data for this analysis was collected from Jargon's Supabase database on March 16, 2025, covering all user interactions since the extension's launch in June 2024. The data was exported as CSV files and processed using R for analysis.
...
# Read the CSV files
profiles <- read.csv("data/profiles_rows.csv")
...
# Display the first few rows and structure of the enhanced dataset
str(enhanced_profiles)
head(enhanced_profiles)
```
...
1. **Data Loading**: Five primary CSV files were imported containing user profiles, questions, words, levels, and blocked websites information.

2. **Data Enhancement**: The profiles dataset was enriched with derived metrics:
   - `generated_questions`: Total number of questions created by each user
   - `answered_questions`: Count of questions completed by each user
   - `blocked_sites`: Number of websites where users disabled the extension
   - `levels_attempted`: Count of unique difficulty levels tried by each user

3. **Missing Value Handling**: NA values in the derived metrics were replaced with 0, assuming no activity is equivalent to zero count.

4. **Data Validation**: The enhanced dataset was examined to ensure all joins were performed correctly and derived metrics were calculated accurately.

This processed dataset forms the foundation for our analysis of user engagement patterns and feature usage in the Jargon extension.
